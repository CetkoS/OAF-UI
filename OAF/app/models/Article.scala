...
case class Article
(id: Option[Long], name: String, description: Option[String], price: Double, pictureLocation: String, weight: Option[Double], companyId: Long, status: ArticleStatus.Value)

object Article {

  implicit def convertToModel(articleDbModel: ArticleDBModel) : Article = {
    Article(articleDbModel.id, articleDbModel.name, articleDbModel.description, articleDbModel.price, articleDbModel.pictureLocation, articleDbModel.weight, articleDbModel.companyId, articleDbModel.status)
  }
